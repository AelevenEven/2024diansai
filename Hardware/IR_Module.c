#include "IR_Module.h"
/* 修改原因：循迹减速计算使用浮点绝对值，应使用 fabsf() 而不是整数/双精度版本。 */
#include <math.h>
...
void IRDM_line_inspection(void)
{
  ...
  /*=========================================================================*
    ...
    // 转向速度越大，基础速度越低
	/*
	 * 修改原因：旧公式在转弯量稍大时会把基础速度降到 0，车辆容易停住并丢线；
	 * 改为随转向量线性降速，并保留 ForwardLimit 最低前进速度。
	 */
	base_speed_mm = BaseSpeed - 0.6f * fabsf(turn_diff);
